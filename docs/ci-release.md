...
The job uploads the installer, SHA-256 text file, three CycloneDX JSON documents, and Playwright report as `AI-Optimization-Tool-0.13.0-windows-rc`. It does not create tags or GitHub Releases. Microsoft Defender scans the installer when the hosted runner exposes its command-line scanner; otherwise the log records that limitation for manual follow-up.
