...
Ingestion uses a configurable per-client sliding-window rate limit and a 5 MB request limit. The `/api/v1/import/*` endpoints use a separate 500 MB request limit to support large-file telemetry imports without affecting the general API. Responses include content-type, framing, referrer, permissions, and content-security headers. Structured logs record method, path, status, and duration without query strings, bodies, credentials, prompts, or responses.
