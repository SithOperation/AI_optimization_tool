...
& $Python -m PyInstaller --noconfirm --clean --onefile --windowed --name aiopt-backend --workpath (Join-Path $BuildRoot 'work') --specpath (Join-Path $BuildRoot 'spec') --distpath (Join-Path $BuildRoot 'dist') --paths (Join-Path $RepoRoot 'apps\api') --paths $RepoRoot --add-data "$(Join-Path $RepoRoot 'VERSION');." --collect-all statsforecast --exclude-module numba.np.ufunc.tbbpool (Join-Path $RepoRoot 'apps\api\desktop_entry.py')
